    ...
    try:
        ...
        while True:
            ...
            if i+segment >= len(words):
                ...
                if i >= next_words:
                    if dataset.index_to_word[word_index] == ("****4", "****3")[args.from_name]:
                        break
...
parser.add_argument('--from-name', action='store_true')
...
checkpoint = torch.load(("modelname.pt", "modelcard.pt")[args.from_name], map_location=torch.device('cpu'))
